...
for(i in 1:nrow_org){
  ...
  if(org_status_end < status_end){
    ...
    while(org_status_end > status_end){
      ...
      if(org_status_end > status_end){
        et <- as.difftime(0,units ="secs") #0分
      }else{
        et <- as.difftime(900,units ="secs") #MAX15分
...
df_times15$useTime <- 900-df_times15$et

# df_times15_bk <- df_times15
# df_times15 <- df_times15_bk
df_times15 <-
df_times15 %>%
  dplyr::group_by(thing_id,datetime) %>%
  dplyr::summarise(et = sum(et),useTime = sum(useTime),n = n())
func_culk_emptime <- function(n,useTime) {
  y <- n * 900
  z <- y-useTime
  z
}
df_times15$culktime <- mapply(function(x, y) {func_culk_emptime(x,y)}, df_times15$n, df_times15$useTime )
df_times15$emptime <- df_times15$n - df_times15$useTime
#df_times15$emptime <- apply(df_times15,2,func_culk_emptime(n,useTime))
